...
ssh $node "sudo yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm"
ssh $node "sudo yum-config-manager --enable rhui-REGION-rhel-server-extras"
